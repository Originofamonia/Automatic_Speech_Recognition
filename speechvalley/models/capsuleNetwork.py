    ...
    def __call__(self, inputX, kernel_size, strides, num_iter, with_routing=True, padding='VALID'):
        ...
        with tf.variable_scope(self._vars_scope) as scope:
            if self._layer_type == 'conv':
                ...
                kernel = tf.get_variable("conv_kernel",
                                         shape=[kernel_size[0], kernel_size[1], input_shape[-1],
                                                self._num_channels * self._num_capsules * self._output_vector_len],
                                         dtype=tf.float32)
                ...
                if with_routing:
                    ...
                    # size of u: [batch_size, channels, num_capsules, output_vector_len]
                    capsule_output = routing(capsule_output, self._num_channels, self._num_capsules,
                                             self._output_vector_len, num_iter, scope)
                capsule_output = squashing(capsule_output)
                # size of capsule_output: [batch_size, num_capsules, num_vector_len, output_vector_len]
                capsule_output = tf.reshape(capsule_output,
                                            [input_shape[0], self._num_capsules, self._output_vector_len,
                                             self._num_channels])
                ...
                capsule_output = routing(inputX, self._num_channels, self._num_capsules, self._output_vector_len,
                                         num_iter, scope)
